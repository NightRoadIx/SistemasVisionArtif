...
ancho = 24.26  # milímetros, aproximadamente 0.9 inches


def midpoint(ptA, ptB):
    """
    Calcula el punto medio entre dos puntos en un plano cartesiano.

    Args:
        ptA (tuple): Coordenadas del primer punto en forma de tupla (x, y).
        ptB (tuple): Coordenadas del segundo punto en forma de tupla (x, y).

    Returns:
        tuple: Coordenadas del punto medio en forma de tupla (x_medio, y_medio).
    """
    return ((ptA[0] + ptB[0]) * 0.5, (ptA[1] + ptB[1]) * 0.5)


# Cargar la imagen, cambiarla a escala de grises y difuminarla un poco
image = cv2.imread("size_of_objects_reference.jpg")
